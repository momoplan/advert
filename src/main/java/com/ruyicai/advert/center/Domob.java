...
public class Domob extends AbstractScoreWall {
	...
	public Map<String, Object> receiveAdvertise(Map<String, String> param) {
		...
		String returnFormat = param.get("returnFormat");
		logger.info("多盟广告点击记录 start mac="+mac+";appId="+appId+";source="+source+";returnFormat="+returnFormat);
		//验证ip
		/*boolean verfyIp = verfyIp(ip, propertiesUtil.getDianru_ip());
		if (!verfyIp) {
			logger.error("多盟广告点击记录,ip不合法 drkey="+drkey+";source="+source+";ip="+ip);
